...
async function handleUpload() {
	...
	for (let i = 0; i < fileList.value.length; i++) {
		...
		try {
			...
			const config: Record<string, any> = {
				...
				onUploadProgress: (progressEvent: any) => {
					if (progressEvent.total) {
						...
						uploadProgress.value = Math.round(((currentIndex + currentProgress) / fileList.value.length) * 100);
					}
				},
			};
			if (props.headers) {
			...
			props.showSuccessMessage && ElMessage.success(`文件"${file.name}"上传成功`);
			results.push(res);
		} catch (err: any) {
			hasError = true;
			props.showErrorMessage && ElMessage.error(`文件"${file.name}"上传失败：${err?.message || "未知错误"}`);
			emit("error", err);
			// props.onError?.(err)
		}
	}
	...
	if (!hasError) {
		...
		if (props.multiple) {
			props.showSuccessMessage && ElMessage.success("所有文件上传完成");
		}

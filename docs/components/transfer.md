...
用于在两个面板之间进行数据交互，支持单选/多选、分组折叠、搜索过滤、远程数据源等功能。

业务项目中请确保已引入组件库样式，否则布局会退化为无样式的块级堆叠：
...
| options | 静态选项列表（优先级高于 remote，支持单层或分组格式） | `TransferOption[] \| TransferGroup[]` | - |
...
| groupNameKey | 分组名称字段名 | `string` | `'label'` |
| groupOptionsKey | 分组选项列表字段名 | `string` | `'options'` |